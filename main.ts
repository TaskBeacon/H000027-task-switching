...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000027-task-switching",
    task_name: "Task Switching Task",
    task_description: "HTML preview aligned to local psyflow Task Switching procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
            ...
            buildWaitTrial(
              ...
              [
                (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
                  const summary = summarizeBlock(runtime.getReducedRows(), blockId, controller.current_score);
      ...
      compiledTrials.push(
        buildWaitTrial(
          ...
          [
            (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
              const summary = summarizeOverall(runtime.getReducedRows(), controller.current_score);
